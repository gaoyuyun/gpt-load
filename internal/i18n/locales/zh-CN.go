...
var MessagesZhCN = map[string]string{
	...
	"config.cooldown_status_codes":           "冷却状态码",
	"config.cooldown_status_codes_desc":      "命中这些 HTTP 状态码时，将当前 key 标记为冷却中。多个状态码可用英文逗号、空格或换行分隔，默认 429。",
	"config.disable_status_codes":            "禁用状态码",
	"config.disable_status_codes_desc":       "命中这些 HTTP 状态码时，将当前 key 自动禁用并记录原因。多个状态码可用英文逗号、空格或换行分隔，默认 402。",
	"config.direct_fail_status_codes":        "直接失败状态码",
	"config.direct_fail_status_codes_desc":   "命中这些 HTTP 状态码时，不再重试其他 key，直接将上游错误返回给客户端，同时不惩罚当前 key。多个状态码可用英文逗号、空格或换行分隔，默认 413。",
