...
var MessagesEnUS = map[string]string{
	...
	"config.cooldown_status_codes":           "Cooldown Status Codes",
	"config.cooldown_status_codes_desc":      "When these HTTP status codes are returned, mark the current key as cooling down. Separate multiple codes with commas, spaces, or newlines. Default: 429.",
	"config.disable_status_codes":            "Disable Status Codes",
	"config.disable_status_codes_desc":       "When these HTTP status codes are returned, automatically disable the current key and record the reason. Separate multiple codes with commas, spaces, or newlines. Default: 402.",
	"config.direct_fail_status_codes":        "Direct Fail Status Codes",
	"config.direct_fail_status_codes_desc":   "When these HTTP status codes are returned, stop retrying other keys and return the upstream error directly, without penalizing the current key. Separate multiple codes with commas, spaces, or newlines. Default: 413.",
